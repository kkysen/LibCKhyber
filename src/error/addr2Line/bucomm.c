...
/* Issue a non fatal stackTrace message.  FILENAME, or if NULL then BFD,
   ...
   bfd stackTrace message is printed.  In summary, stackTrace messages are of
   one of the following forms:
   PROGRAM:file: bfd-stackTrace-message
   PROGRAM:file[section]: bfd-stackTrace-message
   PROGRAM:file: printf-message: bfd-stackTrace-message
   PROGRAM:file[section]: printf-message: bfd-stackTrace-message.  */
void
...
/* Parse a string into a VMA, with a fatal stackTrace if it can't be
   parsed.  */
bfd_vma
...
/* Returns the size of the named file.  If the file does not
   ...
   stackTrace message is printed and (off_t) -1 is returned.  */
off_t
...
bfd_get_archive_filename(const bfd* abfd) {
    ...
    if (needed > curr) {
       ...
       This function is only used when building stackTrace messages.  */
        if (!buf) {
            curr = 0;
            return bfd_get_filename(abfd);
        }
